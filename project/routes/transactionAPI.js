...
async function checkApiKey(req, res, next) {
  ...
  let valid = await Restaurant.findOne({
    where: { api_key: api_key.slice(7) },
  });
  if (valid) {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      let grandTotal = transactions.reduce(
        (sum, trans) => sum + trans.grand_total,
        0
      );

      return res.json({
        "Restaurant Name": req.body.restaurant.nama_restaurant,
        "Transaction Grand Total": grandTotal,
      });
    } catch (error) {
      console.error(error);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      return res.json({
        message: `Empty customer transaction on ${restaurant.nama_restaurant} restaurant`,
      });
    } catch (error) {
      console.error(error);
      return res.status(500).json({ message: "Internal Server Error" });
...
router.get("/api/sumIncome", [checkApiKey, checkLogin], async (req, res) => {
  try {
    ...
    let grandTotalIncome = transactions.reduce(
      (sum, trans) => sum + trans.grand_total,
      0
    );
    ...
    return res.json({
      "Restaurant Name": req.body.restaurant.nama_restaurant,
      "Restaurant Income": grandTotalIncome,
    });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: "Internal Server Error" });
...
router.post("/payment-webhook", async (req, res) => {
  ...
  if (paymentInfo.transaction_status === "capture") {
    if (
      (paymentInfo.order_id.includes("TOPUP") ||
        paymentInfo.order_id.includes("TRANSACTION")) &&
      loggedUser
    ) {
      if (
        ...
        if (owner) {
          owner.api_hit =
            loggedUser.api_hit + parseInt(paymentInfo.gross_amount) / 5;
          await owner.save();
        }
      }
    }
